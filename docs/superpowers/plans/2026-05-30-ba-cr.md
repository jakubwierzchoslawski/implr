...
description: >
  ...
  already been generated. Triggers on: change requirement, update requirement, override
  constraint, reduce scope, add new rule, requirement change request, cr, change request,
  I want to change, limit X to Y, adjust requirement. Interviews the user to capture the
...
Confirm the file contains: frontmatter with trigger phrases, Reference section, Outputs You Own, Parameters with all four flags, all five phases (0–4), Manual-File Trigger section, Tone section.
...
- [ ] **Step 1: Add change-requests detection to PHASE 9 — Report**
...
After completing the report, check whether any NEW files were discovered under
`docs/kb/change-requests/` during this run. For each such file, emit:
...
Emit one line per new CR file. Do not auto-chain ba-cr — the user must trigger it explicitly.
This detection only fires for NEW files (status NEW in Phase 2); already-indexed CR files
are silent.
```

- [ ] **Step 2: Update the description frontmatter to mention ba-cr trigger**

In `skills/doc-ingest/SKILL.md`, locate the frontmatter `description` field. At the end of the description, add the sentence:

```
When new files are found under docs/kb/change-requests/, emits a prompt to run /ba-cr.
```

- [ ] **Step 3: Verify the detection block is positioned correctly**

Confirm the detection block is inside PHASE 9 (after the main report block, before Incremental Guarantees), and that it correctly fires only for NEW files.
...
Confirm: the three state flow tables are present (Requirement, Plan, CR), the CLI path shows 6 steps matching the ba-cr phases, the manual-file path shows 4 steps with the explicit Step 4 note.
...
- `ba-cr` skill: manage Change Requests to amend requirements and plans after generation.
  Supports CLI interview path (`/ba-cr`) and manual-file path (`/ba-cr --file <path>`).
...
- `doc-ingest`: detects new files in `docs/kb/change-requests/` and prompts to run ba-cr
