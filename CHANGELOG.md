...
### Cost reduction Wave 2

- **doc-ingest: inline KB extraction.** `doc-ingest` Phase 3 now runs text extraction
  inline via `Bash` shell commands (`cp`, `pdftotext`, python one-liners). File content
  never enters an LLM context window during extraction. Eliminates one subagent dispatch
  + full file-content token spend per KB file per ingest.
- **ba-requirements-gen: inline envelope dispatch.** Mirrors the v3.0 task-executor
  pattern. `ba-requirements-gen` reads stable files once per run, builds a compact
  `domain_envelope` (requirements-card inline + domain synthesis + NFR excerpt), and
  passes it to each `requirements-domain-worker`. Workers no longer cold-read
  `requirement-schema.md`, `DEV-STANDARDS.md`, `implr.config.yaml`, or
  `master-synthesis.md`. Saves ~30–40k tokens × N domains per run.
- New `docs/implr/config/requirements-card.md` (auto-generated by `/implr-init`):
  compact executable subset of requirement-schema + DEV-STANDARDS NFR baselines + TDD
  mapping, consumed by `requirements-domain-worker`.

### New artefacts
- `docs/implr/config/requirements-card.md` — auto-generated by `/implr-init` and
  `/implr-init --refresh-card`; do not hand-edit.

### Removed agents
| Agent | Reason |
|---|---|
| `doc-ingest-extractor` | Replaced by inline shell extraction in `doc-ingest` skill |

### Updated skills
| Skill | Change |
|---|---|
| `/doc-ingest` | Phase 3 rewritten: inline `Bash` extraction; no subagent dispatch |
| `/ba-requirements-gen` | Phase 3 rewritten: build `domain_envelope` once per run; pass inline to workers |
| `/implr-init` | Step 5 generates `requirements-card.md`; `--refresh-card` regenerates both cards |

### Breaking changes
- `.claude/agents/doc-ingest-extractor.md` removed. Installer auto-cleans on update.
...
### Cost reduction
- New `plan-runner` agent replaces `executor-worker`; no stable reads (~30k overhead
  per plan eliminated).
- `task-executor` now receives an inline task envelope; no longer reads plan-schema,
  ARCHITECTURE.md, DEV-STANDARDS.md, or the full plan (~17k per task eliminated).
- New `arch-excerpter` agent (Sonnet): produces a per-plan architecture excerpt; one call
  per plan amortised across all tasks.
- New `docs/implr/config/standards-card.md` (auto-generated by `/implr-init`): compact
  executable subset of DEV-STANDARDS.md consumed by `task-executor` and `code-review-worker`.
...
- Estimated end-to-end token reduction: **6–10× vs v1.x**, **2.5–3× vs v2.0** per plan.

### Workflow simplifications
- `/doc-ingest` defaults to full pipeline (extract + digest + syntheses). `--registry-only`
  for v2.0 fast-scan behaviour. `--digest` deprecated (no-op with warning).
- `/dev-planner` cross-plan coherence sweep is opt-in (`--coherence-check`) or auto-on
  when ≥3 plans generated in one invocation.
- `/dev-executor` and `/dev-code-review` default reports are one screen; `--verbose` for
  per-plan detail.
- `/dev-executor --review` chains code-review automatically after successful execution.

### New agents
| Agent | Role |
|---|---|
| `plan-runner` | Per-plan dispatcher; replaces `executor-worker`; no stable reads |
| `arch-excerpter` | Sonnet; per-plan ARCHITECTURE excerpt for task envelopes |

### Removed agents
| Agent | Reason |
|---|---|
| `executor-worker` | Replaced by `plan-runner` |

### New artefacts
- `docs/implr/config/standards-card.md` — auto-generated by `/implr-init --refresh-card`
- `docs/implr/DOD.md` — canonical Definition of Done (seeded by installer)

### Breaking changes
- `.claude/agents/executor-worker.md` removed. Installer auto-cleans on update.
- `/doc-ingest` now runs the full pipeline by default (was registry-only in v2.0).
- v2.0 plans with non-compact task headers (`### TASK-NNN:` without `· complexity/TDD · files`) will trigger parse-failure abort in `dev-executor` — regenerate with `/dev-planner --replan`.
